...
serve(async (req) => {
  ...
  try {
    const { owner, repo, branch, message, files, baseBranch = 'main', projectId } = await req.json();
    
    // Validate authentication - projectId is optional for backward compatibility
    // but if provided, verify user has editor+ access (commits require write access)
    if (projectId) {
